...
function UserLogin(e) {
	...
	(login_user_data.password = document.getElementById(
		"login-user-password"
	).value),
		5;
...
function UserPreRegister(e) {
	...
	if (
		reg_user_data.full_name &&
		ValidateEmail(reg_user_data.email) &&
		reg_user_data.password &&
		reg_user_data.birth_date &&
		reg_user_data.birth_date &&
		reg_user_data.gender
	) {
		e.preventDefault();
...
function SendUserDataToServer(user_data, stat, reg_stat) {
	...
	fetch(`${window.origin}/user_sign`, {
		...
		if (response.status !== 200) {
			...
			alert(
				`Some thing went wrong, There is a problem in you data used in ${
					stat == "login" ? "login" : "registration"
				}`
			);
		...
		response.json().then((data) => {
			...
			if (stat == "login") {
				...
				if (reg_stat == "pri_stage" && data["reg_valid"] && !data["user_reg"]) {
					...
					reg_stat = "sec_stage";
				} else if (data["user_reg"]) GoToUserPage(data["username"]);
				else
					alert(
						"There is a problem with regestration, Please try again later."
					);
			}
